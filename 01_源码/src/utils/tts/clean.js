...
// v3.8.357：提示音已由统一音频预处理裁掉，不再需要用长静音掩盖串音；句间停顿整体收紧，
// 让连续讲解保持顺畅，同时保留必要的换气感。
export function speechPauseMs(text) {
  const t = String(text || '').trim()
  if (/[。！？!?…]$/.test(t)) return 150 // 句末：保留换气但不拖沓
  if (/[；;]$/.test(t)) return 105 // 分号：并列分句
  if (/[，,：:、]$/.test(t)) return 65 // 逗号/冒号/顿号：短停顿
  return 40 // 无标点（硬切）：只留最低呼吸感
}
